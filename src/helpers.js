...
export async function getHoldings(address, signingCosmWasmClient) {
    try {
        ...
        let tokensHeld = await signingCosmWasmClient.queryContractSmart("sei1vv2jalkwv5vhk0axnfqfuk7rtnyvrh72wz4tjvraqgf4r83dqdfs22882j", {
            tokens: {
                owner: address
            }
        });

        console.log("You hold " + tokensHeld.tokens.length + " Peaxel.");
        return tokensHeld.tokens.length;
    } catch (error) {
        console.error("Error checking if Peaxel holder:", error);
        return 0;
    }
}
